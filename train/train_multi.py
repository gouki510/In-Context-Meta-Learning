...
from configs.config_multi import TransformerConfig, TrainDataConfig, IWLDataConfig, ICLDataConfig, ICL2DataConfig, MainConfig
...
def main(config, save_path):
    ...
    for (data_dict, icl_data_dict, iwl_data_dict) in zip(tqdm(train_dataloader), icl_dataloader, iwl_dataloader):
        ...
        if step > trainconfig.optimize_step:
            ...
            torch.save(model.state_dict(), os.path.join(save_path, config.exp_name+".pt"))
            break
