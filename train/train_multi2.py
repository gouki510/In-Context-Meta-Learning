...
def main(config):
    wandb.init(project="induction-head-multitask2", config=asdict(config))
    ...
    # model
    embedder = MultiTaskInputEmbedderV1(modelconfig)
...
if __name__ == "__main__":
    ...
    config.modelconfig.n_ctx = (config.traindataconfig.num_seq+1)*3 -1
    
    config.traindataconfig.task_ways = parser.parse_args().task_ways
